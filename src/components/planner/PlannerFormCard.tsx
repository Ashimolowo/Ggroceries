...
import { GgoceriesCategory, GgoceriesPriority, useGgoceriesStore } from "@/store/ggoceries-store";
import { FontAwesome6 } from "@expo/vector-icons";
import { useState } from "react";
import { Alert, Pressable, Text, TextInput, View } from "react-native";

const categories: GgoceriesCategory[] = [
  "Produce",
  "Dairy",
  "Bakery",
  "Pantry",
...
const priorities: GgoceriesPriority[] = ["low", "medium", "high"];
...
const PlannerFormCard = () => {
  const { error, addItem } = useGgoceriesStore();

  const [name, setName] = useState("");
  const [quantity, setQuantity] = useState("1");
  const [category, setCategory] = useState<GgoceriesCategory>("Produce");
  const [priority, setPriority] = useState<GgoceriesPriority>("medium");
